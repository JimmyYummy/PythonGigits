...
def get_the_input():
    '''Return a integer input, the function will keep running
    until the input is an integer'''
...
def take_discard_pile(tower, discard):
    ''' Execute the player's action on the discard pile,
    return true is the brick is taken'''
...
def find_and_replace(new_brick, brick_to_be_replaced, tower, discard):
    ...
    ## try to replace the brick, and put the old brick
    ## on the top of the discard pile, return True if succeed
...
def computer_play(tower, main_pile, discard):
    ...
    The computer first will try to make the top brick to be between 1 and 20,
    then start ordering from top to bottom.

    Step 1: When the tower's top brick is larger than 20,
    the computer will try discard pile first and then the main pile 
    to make the tower top to be smaller than 20.

    Step 2:When the tower's top brick is no larger than 20,
    Name the sequence of bricks from top to bottom that is ordered as S.
    
    If the taken brick is larger than any brick in S,
    the computer will place it under S.
    
    e.g. if the tower is [10,20,12,...],
    and the given top of discard pile is 25,
    computer will replace Brick 12 with 25.
    
    If the taken brick is smaller than at least one brick in S,
    then computer will replace the first brick that is larger than it.
    
    e.g. if the tower is [10,26,30,...],
    and the given top of discard pile is 25,
    the computer will replace Brick 26 with 25.
    '''
